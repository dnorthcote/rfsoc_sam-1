-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\simple_window\window.vhd
-- Created: 2020-10-19 10:59:06
-- 
-- Generated by MATLAB 9.8 and HDL Coder 3.16
-- 
-- 
-- -------------------------------------------------------------
-- Rate and Clocking Details
-- -------------------------------------------------------------
-- Model base rate: -1
-- Target subsystem base rate: -1
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: window
-- Source Path: window
-- Hierarchy Level: 0
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;

ENTITY window IS
  PORT( IPCORE_CLK                        :   IN    std_logic;  -- ufix1
        IPCORE_RESETN                     :   IN    std_logic;  -- ufix1
        AXI4_Stream_Window_Slave_TDATA    :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Stream_Window_Slave_TVALID   :   IN    std_logic;  -- ufix1
        AXI4_Stream_Real_Master_TREADY    :   IN    std_logic;  -- ufix1
        AXI4_Stream_Real_Slave_TDATA      :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Stream_Real_Slave_TVALID     :   IN    std_logic;  -- ufix1
        AXI4_Stream_Imag_Master_TREADY    :   IN    std_logic;  -- ufix1
        AXI4_Stream_Imag_Slave_TDATA      :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Stream_Imag_Slave_TVALID     :   IN    std_logic;  -- ufix1
        AXI4_Lite_ACLK                    :   IN    std_logic;  -- ufix1
        AXI4_Lite_ARESETN                 :   IN    std_logic;  -- ufix1
        AXI4_Lite_AWADDR                  :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Lite_AWVALID                 :   IN    std_logic;  -- ufix1
        AXI4_Lite_WDATA                   :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
        AXI4_Lite_WSTRB                   :   IN    std_logic_vector(3 DOWNTO 0);  -- ufix4
        AXI4_Lite_WVALID                  :   IN    std_logic;  -- ufix1
        AXI4_Lite_BREADY                  :   IN    std_logic;  -- ufix1
        AXI4_Lite_ARADDR                  :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Lite_ARVALID                 :   IN    std_logic;  -- ufix1
        AXI4_Lite_RREADY                  :   IN    std_logic;  -- ufix1
        AXI4_Stream_Window_Slave_TREADY   :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Real_Master_TDATA     :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Stream_Real_Master_TVALID    :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Real_Master_TLAST     :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Real_Master_TUSER     :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Real_Slave_TREADY     :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Imag_Master_TDATA     :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
        AXI4_Stream_Imag_Master_TVALID    :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Imag_Master_TLAST     :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Imag_Master_TUSER     :   OUT   std_logic;  -- ufix1
        AXI4_Stream_Imag_Slave_TREADY     :   OUT   std_logic;  -- ufix1
        AXI4_Lite_AWREADY                 :   OUT   std_logic;  -- ufix1
        AXI4_Lite_WREADY                  :   OUT   std_logic;  -- ufix1
        AXI4_Lite_BRESP                   :   OUT   std_logic_vector(1 DOWNTO 0);  -- ufix2
        AXI4_Lite_BVALID                  :   OUT   std_logic;  -- ufix1
        AXI4_Lite_ARREADY                 :   OUT   std_logic;  -- ufix1
        AXI4_Lite_RDATA                   :   OUT   std_logic_vector(31 DOWNTO 0);  -- ufix32
        AXI4_Lite_RRESP                   :   OUT   std_logic_vector(1 DOWNTO 0);  -- ufix2
        AXI4_Lite_RVALID                  :   OUT   std_logic  -- ufix1
        );
END window;


ARCHITECTURE rtl OF window IS

  -- Component Declarations
  COMPONENT window_axi_lite
    PORT( reset                           :   IN    std_logic;
          AXI4_Lite_ACLK                  :   IN    std_logic;  -- ufix1
          AXI4_Lite_ARESETN               :   IN    std_logic;  -- ufix1
          AXI4_Lite_AWADDR                :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Lite_AWVALID               :   IN    std_logic;  -- ufix1
          AXI4_Lite_WDATA                 :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          AXI4_Lite_WSTRB                 :   IN    std_logic_vector(3 DOWNTO 0);  -- ufix4
          AXI4_Lite_WVALID                :   IN    std_logic;  -- ufix1
          AXI4_Lite_BREADY                :   IN    std_logic;  -- ufix1
          AXI4_Lite_ARADDR                :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Lite_ARVALID               :   IN    std_logic;  -- ufix1
          AXI4_Lite_RREADY                :   IN    std_logic;  -- ufix1
          read_ip_timestamp               :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          AXI4_Lite_AWREADY               :   OUT   std_logic;  -- ufix1
          AXI4_Lite_WREADY                :   OUT   std_logic;  -- ufix1
          AXI4_Lite_BRESP                 :   OUT   std_logic_vector(1 DOWNTO 0);  -- ufix2
          AXI4_Lite_BVALID                :   OUT   std_logic;  -- ufix1
          AXI4_Lite_ARREADY               :   OUT   std_logic;  -- ufix1
          AXI4_Lite_RDATA                 :   OUT   std_logic_vector(31 DOWNTO 0);  -- ufix32
          AXI4_Lite_RRESP                 :   OUT   std_logic_vector(1 DOWNTO 0);  -- ufix2
          AXI4_Lite_RVALID                :   OUT   std_logic;  -- ufix1
          write_axi_enable                :   OUT   std_logic;  -- ufix1
          write_enable                    :   OUT   std_logic;  -- ufix1
          reset_internal                  :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_reset_sync
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset_in                        :   IN    std_logic;  -- ufix1
          reset_out                       :   OUT   std_logic
          );
  END COMPONENT;

  COMPONENT window_axi4_stream_window_slave
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          AXI4_Stream_Window_Slave_TDATA  :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Stream_Window_Slave_TVALID :   IN    std_logic;  -- ufix1
          AXI4_Stream_Window_Slave_TREADY :   OUT   std_logic;  -- ufix1
          user_data                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
          user_valid                      :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_axi4_stream_real_slave
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          AXI4_Stream_Real_Slave_TDATA    :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Stream_Real_Slave_TVALID   :   IN    std_logic;  -- ufix1
          AXI4_Stream_Real_Slave_TREADY   :   OUT   std_logic;  -- ufix1
          user_data                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
          user_valid                      :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_axi4_stream_imag_slave
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          AXI4_Stream_Imag_Slave_TDATA    :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Stream_Imag_Slave_TVALID   :   IN    std_logic;  -- ufix1
          AXI4_Stream_Imag_Slave_TREADY   :   OUT   std_logic;  -- ufix1
          user_data                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
          user_valid                      :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_dut
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          dut_enable                      :   IN    std_logic;  -- ufix1
          enable                          :   IN    std_logic;  -- ufix1
          s_window_valid                  :   IN    std_logic;  -- ufix1
          s_window_data                   :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          s_real_valid                    :   IN    std_logic;  -- ufix1
          s_real_data                     :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En15
          s_imag_valid                    :   IN    std_logic;  -- ufix1
          s_imag_data                     :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En15
          ce_out                          :   OUT   std_logic;  -- ufix1
          m_real_valid                    :   OUT   std_logic;  -- ufix1
          m_real_data                     :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En15
          m_real_user                     :   OUT   std_logic;  -- ufix1
          m_real_last                     :   OUT   std_logic;  -- ufix1
          m_imag_valid                    :   OUT   std_logic;  -- ufix1
          m_imag_data                     :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En15
          m_imag_user                     :   OUT   std_logic;  -- ufix1
          m_imag_last                     :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_axi4_stream_real_master
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          AXI4_Stream_Real_Master_TREADY  :   IN    std_logic;  -- ufix1
          user_data                       :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          user_valid                      :   IN    std_logic;  -- ufix1
          user_TLAST                      :   IN    std_logic;  -- ufix1
          user_TUSER                      :   IN    std_logic;  -- ufix1
          AXI4_Stream_Real_Master_TDATA   :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Stream_Real_Master_TVALID  :   OUT   std_logic;  -- ufix1
          AXI4_Stream_Real_Master_TLAST   :   OUT   std_logic;  -- ufix1
          AXI4_Stream_Real_Master_TUSER   :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  COMPONENT window_axi4_stream_imag_master
    PORT( clk                             :   IN    std_logic;  -- ufix1
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          AXI4_Stream_Imag_Master_TREADY  :   IN    std_logic;  -- ufix1
          user_data                       :   IN    std_logic_vector(15 DOWNTO 0);  -- ufix16
          user_valid                      :   IN    std_logic;  -- ufix1
          user_TLAST                      :   IN    std_logic;  -- ufix1
          user_TUSER                      :   IN    std_logic;  -- ufix1
          AXI4_Stream_Imag_Master_TDATA   :   OUT   std_logic_vector(15 DOWNTO 0);  -- ufix16
          AXI4_Stream_Imag_Master_TVALID  :   OUT   std_logic;  -- ufix1
          AXI4_Stream_Imag_Master_TLAST   :   OUT   std_logic;  -- ufix1
          AXI4_Stream_Imag_Master_TUSER   :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : window_axi_lite
    USE ENTITY work.window_axi_lite(rtl);

  FOR ALL : window_reset_sync
    USE ENTITY work.window_reset_sync(rtl);

  FOR ALL : window_axi4_stream_window_slave
    USE ENTITY work.window_axi4_stream_window_slave(rtl);

  FOR ALL : window_axi4_stream_real_slave
    USE ENTITY work.window_axi4_stream_real_slave(rtl);

  FOR ALL : window_axi4_stream_imag_slave
    USE ENTITY work.window_axi4_stream_imag_slave(rtl);

  FOR ALL : window_dut
    USE ENTITY work.window_dut(rtl);

  FOR ALL : window_axi4_stream_real_master
    USE ENTITY work.window_axi4_stream_real_master(rtl);

  FOR ALL : window_axi4_stream_imag_master
    USE ENTITY work.window_axi4_stream_imag_master(rtl);

  -- Signals
  SIGNAL reset                            : std_logic;
  SIGNAL const_1                          : std_logic;  -- ufix1
  SIGNAL reset_cm                         : std_logic;  -- ufix1
  SIGNAL ip_timestamp                     : unsigned(31 DOWNTO 0);  -- ufix32
  SIGNAL AXI4_Lite_BRESP_tmp              : std_logic_vector(1 DOWNTO 0);  -- ufix2
  SIGNAL AXI4_Lite_RDATA_tmp              : std_logic_vector(31 DOWNTO 0);  -- ufix32
  SIGNAL AXI4_Lite_RRESP_tmp              : std_logic_vector(1 DOWNTO 0);  -- ufix2
  SIGNAL write_axi_enable                 : std_logic;  -- ufix1
  SIGNAL write_enable                     : std_logic;  -- ufix1
  SIGNAL reset_internal                   : std_logic;  -- ufix1
  SIGNAL reset_before_sync                : std_logic;  -- ufix1
  SIGNAL top_user_data                    : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL top_user_valid                   : std_logic;  -- ufix1
  SIGNAL top_user_data_unsigned           : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL s_window_data_sig                : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL top_user_data_1                  : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL top_user_valid_1                 : std_logic;  -- ufix1
  SIGNAL top_user_data_unsigned_1         : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL s_real_data_sig                  : signed(15 DOWNTO 0);  -- sfix16_En15
  SIGNAL top_user_data_2                  : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL top_user_valid_2                 : std_logic;  -- ufix1
  SIGNAL top_user_data_unsigned_2         : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL s_imag_data_sig                  : signed(15 DOWNTO 0);  -- sfix16_En15
  SIGNAL ce_out_sig                       : std_logic;  -- ufix1
  SIGNAL m_real_valid_sig                 : std_logic;  -- ufix1
  SIGNAL m_real_data_sig                  : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL m_real_user_sig                  : std_logic;  -- ufix1
  SIGNAL m_real_last_sig                  : std_logic;  -- ufix1
  SIGNAL m_imag_valid_sig                 : std_logic;  -- ufix1
  SIGNAL m_imag_data_sig                  : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL m_imag_user_sig                  : std_logic;  -- ufix1
  SIGNAL m_imag_last_sig                  : std_logic;  -- ufix1
  SIGNAL m_real_data_sig_signed           : signed(15 DOWNTO 0);  -- sfix16_En15
  SIGNAL top_user_data_3                  : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL AXI4_Stream_Real_Master_TDATA_tmp : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL m_imag_data_sig_signed           : signed(15 DOWNTO 0);  -- sfix16_En15
  SIGNAL top_user_data_4                  : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL AXI4_Stream_Imag_Master_TDATA_tmp : std_logic_vector(15 DOWNTO 0);  -- ufix16

BEGIN
  u_window_axi_lite_inst : window_axi_lite
    PORT MAP( reset => reset,
              AXI4_Lite_ACLK => AXI4_Lite_ACLK,  -- ufix1
              AXI4_Lite_ARESETN => AXI4_Lite_ARESETN,  -- ufix1
              AXI4_Lite_AWADDR => AXI4_Lite_AWADDR,  -- ufix16
              AXI4_Lite_AWVALID => AXI4_Lite_AWVALID,  -- ufix1
              AXI4_Lite_WDATA => AXI4_Lite_WDATA,  -- ufix32
              AXI4_Lite_WSTRB => AXI4_Lite_WSTRB,  -- ufix4
              AXI4_Lite_WVALID => AXI4_Lite_WVALID,  -- ufix1
              AXI4_Lite_BREADY => AXI4_Lite_BREADY,  -- ufix1
              AXI4_Lite_ARADDR => AXI4_Lite_ARADDR,  -- ufix16
              AXI4_Lite_ARVALID => AXI4_Lite_ARVALID,  -- ufix1
              AXI4_Lite_RREADY => AXI4_Lite_RREADY,  -- ufix1
              read_ip_timestamp => std_logic_vector(ip_timestamp),  -- ufix32
              AXI4_Lite_AWREADY => AXI4_Lite_AWREADY,  -- ufix1
              AXI4_Lite_WREADY => AXI4_Lite_WREADY,  -- ufix1
              AXI4_Lite_BRESP => AXI4_Lite_BRESP_tmp,  -- ufix2
              AXI4_Lite_BVALID => AXI4_Lite_BVALID,  -- ufix1
              AXI4_Lite_ARREADY => AXI4_Lite_ARREADY,  -- ufix1
              AXI4_Lite_RDATA => AXI4_Lite_RDATA_tmp,  -- ufix32
              AXI4_Lite_RRESP => AXI4_Lite_RRESP_tmp,  -- ufix2
              AXI4_Lite_RVALID => AXI4_Lite_RVALID,  -- ufix1
              write_axi_enable => write_axi_enable,  -- ufix1
              write_enable => write_enable,  -- ufix1
              reset_internal => reset_internal  -- ufix1
              );

  u_window_reset_sync_inst : window_reset_sync
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset_in => reset_before_sync,  -- ufix1
              reset_out => reset
              );

  u_window_axi4_stream_window_slave_inst : window_axi4_stream_window_slave
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              enb => const_1,
              AXI4_Stream_Window_Slave_TDATA => AXI4_Stream_Window_Slave_TDATA,  -- ufix16
              AXI4_Stream_Window_Slave_TVALID => AXI4_Stream_Window_Slave_TVALID,  -- ufix1
              AXI4_Stream_Window_Slave_TREADY => AXI4_Stream_Window_Slave_TREADY,  -- ufix1
              user_data => top_user_data,  -- ufix16
              user_valid => top_user_valid  -- ufix1
              );

  u_window_axi4_stream_real_slave_inst : window_axi4_stream_real_slave
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              enb => const_1,
              AXI4_Stream_Real_Slave_TDATA => AXI4_Stream_Real_Slave_TDATA,  -- ufix16
              AXI4_Stream_Real_Slave_TVALID => AXI4_Stream_Real_Slave_TVALID,  -- ufix1
              AXI4_Stream_Real_Slave_TREADY => AXI4_Stream_Real_Slave_TREADY,  -- ufix1
              user_data => top_user_data_1,  -- ufix16
              user_valid => top_user_valid_1  -- ufix1
              );

  u_window_axi4_stream_imag_slave_inst : window_axi4_stream_imag_slave
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              enb => const_1,
              AXI4_Stream_Imag_Slave_TDATA => AXI4_Stream_Imag_Slave_TDATA,  -- ufix16
              AXI4_Stream_Imag_Slave_TVALID => AXI4_Stream_Imag_Slave_TVALID,  -- ufix1
              AXI4_Stream_Imag_Slave_TREADY => AXI4_Stream_Imag_Slave_TREADY,  -- ufix1
              user_data => top_user_data_2,  -- ufix16
              user_valid => top_user_valid_2  -- ufix1
              );

  u_window_dut_inst : window_dut
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              dut_enable => write_axi_enable,  -- ufix1
              enable => write_enable,  -- ufix1
              s_window_valid => top_user_valid,  -- ufix1
              s_window_data => std_logic_vector(s_window_data_sig),  -- sfix16_En14
              s_real_valid => top_user_valid_1,  -- ufix1
              s_real_data => std_logic_vector(s_real_data_sig),  -- sfix16_En15
              s_imag_valid => top_user_valid_2,  -- ufix1
              s_imag_data => std_logic_vector(s_imag_data_sig),  -- sfix16_En15
              ce_out => ce_out_sig,  -- ufix1
              m_real_valid => m_real_valid_sig,  -- ufix1
              m_real_data => m_real_data_sig,  -- sfix16_En15
              m_real_user => m_real_user_sig,  -- ufix1
              m_real_last => m_real_last_sig,  -- ufix1
              m_imag_valid => m_imag_valid_sig,  -- ufix1
              m_imag_data => m_imag_data_sig,  -- sfix16_En15
              m_imag_user => m_imag_user_sig,  -- ufix1
              m_imag_last => m_imag_last_sig  -- ufix1
              );

  u_window_axi4_stream_real_master_inst : window_axi4_stream_real_master
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              enb => const_1,
              AXI4_Stream_Real_Master_TREADY => AXI4_Stream_Real_Master_TREADY,  -- ufix1
              user_data => std_logic_vector(top_user_data_3),  -- ufix16
              user_valid => m_real_valid_sig,  -- ufix1
              user_TLAST => m_real_last_sig,  -- ufix1
              user_TUSER => m_real_user_sig,  -- ufix1
              AXI4_Stream_Real_Master_TDATA => AXI4_Stream_Real_Master_TDATA_tmp,  -- ufix16
              AXI4_Stream_Real_Master_TVALID => AXI4_Stream_Real_Master_TVALID,  -- ufix1
              AXI4_Stream_Real_Master_TLAST => AXI4_Stream_Real_Master_TLAST,  -- ufix1
              AXI4_Stream_Real_Master_TUSER => AXI4_Stream_Real_Master_TUSER  -- ufix1
              );

  u_window_axi4_stream_imag_master_inst : window_axi4_stream_imag_master
    PORT MAP( clk => IPCORE_CLK,  -- ufix1
              reset => reset,
              enb => const_1,
              AXI4_Stream_Imag_Master_TREADY => AXI4_Stream_Imag_Master_TREADY,  -- ufix1
              user_data => std_logic_vector(top_user_data_4),  -- ufix16
              user_valid => m_imag_valid_sig,  -- ufix1
              user_TLAST => m_imag_last_sig,  -- ufix1
              user_TUSER => m_imag_user_sig,  -- ufix1
              AXI4_Stream_Imag_Master_TDATA => AXI4_Stream_Imag_Master_TDATA_tmp,  -- ufix16
              AXI4_Stream_Imag_Master_TVALID => AXI4_Stream_Imag_Master_TVALID,  -- ufix1
              AXI4_Stream_Imag_Master_TLAST => AXI4_Stream_Imag_Master_TLAST,  -- ufix1
              AXI4_Stream_Imag_Master_TUSER => AXI4_Stream_Imag_Master_TUSER  -- ufix1
              );

  const_1 <= '1';

  reset_cm <=  NOT IPCORE_RESETN;

  ip_timestamp <= to_unsigned(2010191059, 32);

  reset_before_sync <= reset_cm OR reset_internal;

  top_user_data_unsigned <= unsigned(top_user_data);

  s_window_data_sig <= signed(top_user_data_unsigned);

  top_user_data_unsigned_1 <= unsigned(top_user_data_1);

  s_real_data_sig <= signed(top_user_data_unsigned_1);

  top_user_data_unsigned_2 <= unsigned(top_user_data_2);

  s_imag_data_sig <= signed(top_user_data_unsigned_2);

  m_real_data_sig_signed <= signed(m_real_data_sig);

  top_user_data_3 <= unsigned(m_real_data_sig_signed);

  m_imag_data_sig_signed <= signed(m_imag_data_sig);

  top_user_data_4 <= unsigned(m_imag_data_sig_signed);

  AXI4_Stream_Real_Master_TDATA <= AXI4_Stream_Real_Master_TDATA_tmp;

  AXI4_Stream_Imag_Master_TDATA <= AXI4_Stream_Imag_Master_TDATA_tmp;

  AXI4_Lite_BRESP <= AXI4_Lite_BRESP_tmp;

  AXI4_Lite_RDATA <= AXI4_Lite_RDATA_tmp;

  AXI4_Lite_RRESP <= AXI4_Lite_RRESP_tmp;

END rtl;

